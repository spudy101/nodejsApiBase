...
class NotificationEmitter extends EventEmitter {
  ...
  async initialize() {
    ...
    try {
      // Importar dinámicamente DESPUÉS de que Redis esté conectado
      const redisClient = require('./redis.util');
      
      if (redisClient.isAvailable()) {
        // Crear clientes separados para pub/sub
...
process.on('SIGTERM', async () => {
  await notificationEmitter.close();
});
