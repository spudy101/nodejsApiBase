...
const NotificationSchedulerService = require('../core/services/notificationScheduler.service');
const { logger } = require('@abundbank/shared');

/**
 * Worker de notificaciones
 * Ejecuta tareas programadas usando los services
 */
  ...
  static iniciar() {
    ...
    cron.schedule('*/5 * * * *', async () => {
      try {
        ...
        await NotificationSchedulerService.procesarProgramadas();
      } catch (error) {
    ...
    cron.schedule('*/10 * * * *', async () => {
      try {
        ...
        await NotificationSchedulerService.reintentarPush();
      } catch (error) {
    ...
    cron.schedule('*/15 * * * *', async () => {
      try {
        ...
        await NotificationSchedulerService.reintentarEmail();
      } catch (error) {
    ...
    cron.schedule('0 3 * * *', async () => {
      try {
        ...
        await NotificationSchedulerService.limpiarAntiguas();
      } catch (error) {
