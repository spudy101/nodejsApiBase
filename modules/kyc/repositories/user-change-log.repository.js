...
const { UserChangeLog } = require('../../../shared/models');
...
class UserChangeLogRepository extends BaseRepository {
  ...
  /**
   * Busca logs con paginación y búsqueda
   * @param {Object} filters - { changeType, changedByRole, user_id }
  ...
  async findAllPaginated(filters = {}, paginationParams = {}, searchTerm = null) {
    ...
    if (filters.changeType) {
      criteria.change_type = filters.changeType;
    }

    if (filters.changedByRole) {
      criteria.changed_by_role = filters.changedByRole;
    }
    ...
    const searchConfig = {
      searchTerm,
      searchFields: ['change_reason', 'previous_value', 'new_value'],
    };
