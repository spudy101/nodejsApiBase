...
const { VerificationCode } = require('../../../shared/models');
...
class VerificationCodeRepository extends BaseRepository {
  ...
  async markAsVerified(verificationCodeId, options = {}, verifiedAt = new Date()) {
    const dateToUse = verifiedAt || new Date();
    
    // Validación adicional por seguridad
    if (!(dateToUse instanceof Date) || isNaN(dateToUse.getTime())) {
      logger.warn('Invalid date provided to markAsVerified, using current date', {
        verificationCodeId,
        providedDate: verifiedAt
      });
      return await this.update(verificationCodeId, { verified_at: new Date() }, options);
    }
    
    return await this.update(verificationCodeId, { verified_at: dateToUse }, options);
  }
