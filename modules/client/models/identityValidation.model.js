...
module.exports = (sequelize) => {
  ...
  IdentityValidation.prototype.isPending = function() {
    return this.status === 'pending';
  };

  IdentityValidation.prototype.isCompleted = function() {
    return ['signed', 'cancelled', 'failed', 'expired'].includes(this.status);
  };

  IdentityValidation.prototype.incrementAttempts = async function() {
    this.attempts += 1;
    this.last_attempt_at = new Date();
    await this.save();
  };

  IdentityValidation.prototype.markAsCompleted = async function(status, errorMessage = null) {
    this.status = status;
